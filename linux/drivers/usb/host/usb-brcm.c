...
static int brcm_usb_instance_probe(struct platform_device *pdev)
{
	...
	priv->ctrl_regs = devm_ioremap_resource(&pdev->dev, &ctrl_res);
	...
	if (!of_address_to_resource(dn, 1, &xhci_ec_res)) {
		priv->xhci_ec_regs =
			devm_ioremap_resource(&pdev->dev, &xhci_ec_res);
		if (IS_ERR(priv->xhci_ec_regs)) {
			dev_err(&pdev->dev, "can't map ec register space\n");
			return PTR_ERR(priv->xhci_ec_regs);
		}
	}
